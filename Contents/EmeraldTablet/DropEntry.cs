...
namespace MatterRecord.Contents.EmeraldTablet
{
    ...
    {
        private DropInfo _drop;
        private Texture2D _cachedTexture;
        private Rectangle _frame;
        private string _itemName;
        private const int IconSize = 32;
        private int _factor;
        private int? _bannersPerItem;
        private int? _itemsPerBanner;
        private Action<DropInfo, int, int> _onExchange;
        private bool _canExchange;
        private bool _forceUnavailable;
        private bool _isBagMode;
        private double _bagTotalRate;
        ...
        public DropEntry(DropInfo drop, bool isBagMode, int factor, bool forceUnavailable, double bagTotalRate, Action<DropInfo, int, int> onExchange)
        {
            ...
            Main.instance.LoadItem(drop.ItemID);
            _cachedTexture = TextureAssets.Item[drop.ItemID].Value;
            if (Main.itemAnimations[drop.ItemID] != null)
                _frame = Main.itemAnimations[drop.ItemID].GetFrame(_cachedTexture);
            else
                _frame = _cachedTexture.Bounds;
