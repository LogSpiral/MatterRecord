...
{
    private record struct NPCBannerInfo(int npcId, string npcName, int killsNeeded);
    ...
    private bool _pendingUpdateList;
    ...
    private static Dictionary<int, NPCBannerInfo> _bannerToNPC;
    ...
    public static void BuildBannerDictionary()
    {
        ...
        for (int npcId = -10; npcId < NPCLoader.NPCCount; npcId++)
        {
            ...
            if (bannerId > 0)
            {
                ...
                {
                    _bannerToNPC.Add(bannerItemId, new(npcId, npcName, killsNeeded));
                }
            }
        }
    }
    ...
    public override void Update(GameTime gameTime)
    {
        if (_pendingUpdateList)
        {
            _pendingUpdateList = false;
            if (_isBagMode)
                RefreshDropsForBag();
            else
