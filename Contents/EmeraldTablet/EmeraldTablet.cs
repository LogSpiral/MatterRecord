...
namespace MatterRecord.Contents.EmeraldTablet
{
    ...
    {
        public static ConsumeItemCallback EmeraldTabltAlchemyMethod = (Recipe recipe, int type, ref int amount) =>
        {
            bool flag = false;
        ...
        public override void PostAddRecipes()
        {
            foreach (var recipe in Main.recipe)
            {
                if (recipe.requiredTile.Contains(TileID.Bottles))
                    recipe.AddConsumeItemCallback(EmeraldTabltAlchemyMethod);
            }
            base.PostAddRecipes();
        }
    }
