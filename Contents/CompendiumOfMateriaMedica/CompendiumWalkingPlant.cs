...
namespace MatterRecord.Contents.CompendiumOfMateriaMedica
{
    ...
    public class CompendiumWalkingPlant : ModPlayer
    {
        ...
        private static readonly Dictionary<int, int> TileToHerb = new Dictionary<int, int>();

        // 所有草药样式列表（用于种植盆随机生成）
        private static readonly int[] AllHerbStyles = new int[]
        {
            0, // 太阳花
            1, // 月光草
            2, // 闪耀根
        ...
        /// 判断物品是否为草药种子。
        /// </summary>
        private bool IsHerbSeed(int type)
        {
        ...
        /// 在指定位置放置草药图格。
        /// </summary>
        private void PlaceHerb(int x, int y, int tileId, int style)
        {
